...
import hdbscan  # https://scikit-learn.org/stable/modules/generated/sklearn.cluster.HDBSCAN.html
...
class SemanticChunker:
    ...
    def _cluster_and_process(self, texts, min_size):
        ...
        for i, label in enumerate(labels):
            if label != -1:
                cluster[label].append(texts[i])
            else:
                orphans.append(texts[i])
                
        chunks = []
        for cluster_paras in cluster.values():
            current_chunk = []
            current_tokens = 0
